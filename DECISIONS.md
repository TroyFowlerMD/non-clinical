...
Consequences: Future scheduler request triage should read `docs/psych-scheduler-it-request-inbox.md` and the `Feedback` tab first, then use the token-protected status update operation when a request is triaged or completed. Apps Script deployments should update the existing web app deployment so `DRIVE_EXEC_URL` remains stable.
